...
test('mobile menu opens, locks body scroll and closes after route selection', async ({ page }) => {
  ...
  await expect(mobileNav.getByRole('link', { name: 'Como trabalhamos', exact: true })).toHaveAttribute('href', '/#como-trabalhamos');
  await expect(mobileNav.getByRole('link', { name: 'Produtos', exact: true })).toHaveAttribute('href', '/produtos');
  await expect(mobileNav.getByRole('link', { name: 'Serviços', exact: true })).toHaveAttribute('href', '/empresas');
  ...
  await mobileNav.getByRole('link', { name: 'Como trabalhamos', exact: true }).click();
