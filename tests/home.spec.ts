...
test('carrega a homepage', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Transformamos tarefas complicadas em\s*sistemas simples de utilizar\./i })).toBeVisible();
  ...
  await expect(page.getByTestId('hero-brand-visual').getByText('Pedido recebido')).toBeVisible();
