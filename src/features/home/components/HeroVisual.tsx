  ...
  return (
    ...
    >
      <div
        ...
        className="relative overflow-hidden rounded-[1.65rem] border border-white/12 bg-[#07101F] p-4 shadow-[0_28px_70px_rgba(0,0,0,0.34)] sm:p-5 lg:p-6"
      >
        ...
        <div className="relative">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <span className="rounded-full border border-white/12 bg-white/[0.05] px-3 py-1 text-xs font-semibold text-white/75">
              {categoryLabel}
            </span>
          </div>

          <p id="hero-product-proof-summary" className="mt-3 max-w-[34rem] text-sm leading-6 text-white/65">
            {summary}
          </p>
          ...
          <div className="mt-5 rounded-[1.3rem] border border-white/10 bg-[#0B1728] p-3 sm:p-4">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/8 pb-3">
              <div>
                <p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-white/45">Visão operacional</p>
            ...
            <div className="mt-3 grid gap-2.5">
              {rows.map((row) => (
                ...
                >
                  <p className="min-w-0 text-sm font-medium text-white/78">{row.label}</p>
            ...
            {['Serviço', 'Check-in', 'Evidência', 'Relatório'].map((step, index) => (
              <div key={step} className="min-w-0">
                <div className="flex items-center" aria-hidden="true">
                  ...
                  {index < 3 ? <span className="h-px min-w-0 flex-1 bg-white/12" /> : null}
                </div>
                <p className="mt-1.5 truncate text-[0.68rem] font-medium text-white/55">{step}</p>
              </div>
            ))}
          </div>
        </div>
