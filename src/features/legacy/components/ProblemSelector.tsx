    ...
    <section id="problemas" className="soft-section-line bg-paper py-16 sm:py-20 lg:py-24">
      <div className="container-section">
        <SectionHeading
          ...
          subtitle="Escolha uma situação. A KAVTRIS traduz o problema para uma solução simples, sem exigir que conheça termos técnicos."
          align="center"
        />
          ...
          {businessProblems.map((problem) => {
            ...
            return (
              <button
                ...
                className={`group min-h-[13rem] rounded-[1.35rem] border bg-white p-5 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-card focus:outline-none focus-visible:ring-2 focus-visible:ring-kavtris-blue focus-visible:ring-offset-2 ${
                  isActive ? 'border-kavtris-blue shadow-card' : 'border-borderline'
                }`}
              >
                <span className={`grid h-12 w-12 place-items-center rounded-2xl ${isActive ? 'bg-navy-900 text-kavtris-blueLight' : 'bg-mist text-kavtris-blue'}`}>
        ...
        <div className="mt-8 grid overflow-hidden rounded-[1.6rem] border border-navy-900/15 bg-navy-950 text-white shadow-card lg:grid-cols-[0.95fr_1.05fr]">
          <div className="p-6 sm:p-8">
            <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-kavtris-blueLight">Solução simples</p>
          ...
          <div className="border-t border-white/10 bg-[#08223A] p-6 sm:p-8 lg:border-l lg:border-t-0">
            ...
            <div className="mt-6 rounded-2xl border border-kavtris-blue/25 bg-[#03182B] p-4">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-kavtris-blueLight">Exemplo visual</p>
              <ol className="mt-4 grid gap-3">
                {active.example.map((item, index) => (
                  <li key={item} className="flex items-center gap-3 text-sm text-white/78">
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-kavtris-blue text-xs font-bold text-navy-950">{index + 1}</span>
                    {item}
                  </li>
                ))}
              </ol>
